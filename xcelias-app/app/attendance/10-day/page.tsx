...
export default function TenDayAttendancePage() {
  ...
  const filteredTrainees = form.batchId
    ...
    : trainees;
  const atRiskCount = records.filter((record) => {
    const absentCount = record.absentCount ?? 0;
    const lateCount = record.lateCount ?? 0;
    const completionPercent = record.completionPercent ?? 0;

    return record.attendanceEligible && (completionPercent < 80 || absentCount >= 2 || lateCount >= 2 || record.checklistStatus !== "Complete");
  }).length;
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <Breadcrumb items={[{ label: "Command Center", href: "/command-center" }, { label: "Attendance" }, { label: "10-Day Summary" }]} />
              ...
              <div className="flex items-center gap-3">
                <Link href="/report-studio?template=batch-intervention-pack&source=attendance" className="inline-flex">
            ...
            <div className="glass-card p-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.24em] text-[#57534e]">Connected retention flow</p>
                  <h2 className="mt-2 text-xl font-semibold text-[#fafaf9]">{atRiskCount} attendance record{atRiskCount === 1 ? "" : "s"} need intervention follow-up</h2>
                  <p className="mt-2 max-w-3xl text-sm leading-7 text-[#78716c]">
                    Attendance issues can now hand off directly into learning recovery and reporting, instead of staying isolated inside raw checklist status.
                  </p>
                </div>
                <div className="flex flex-wrap gap-3">
                  <Link href="/learning-hub?focus=attendance-recovery&source=attendance" className="rounded-2xl border border-[#a8a29e]/8 bg-[#231f1d]/40 px-4 py-2 text-sm text-[#fafaf9] transition hover:bg-[#231f1d]">
                    Open catch-up learning
                  </Link>
                  <Link href="/report-studio?template=batch-intervention-pack&source=attendance" className="rounded-2xl bg-[var(--brand-gradient)] px-4 py-2 text-sm font-semibold text-[#041019]">
                    Build intervention pack
                  </Link>
                </div>
              </div>
            </div>
...
function TenDayCard({
  ...
  const needsIntervention = record.attendanceEligible && ((record.completionPercent ?? 0) < 80 || (record.absentCount ?? 0) >= 2 || (record.lateCount ?? 0) >= 2 || record.checklistStatus !== "Complete");
  const traineeParam = encodeURIComponent(record.trainee.traineeName);
  const batchParam = encodeURIComponent(record.batch.batchName);
  ...
  return (
    ...
    >
      ...
      {needsIntervention && (
        <div className="mt-4 rounded-xl border border-[#a8a29e]/6 bg-[#1c1917]/50 p-4">
          <p className="text-xs text-[#57534e]">Recommended next move</p>
          <div className="mt-3 space-y-2">
            <Link
              href={`/learning-hub?focus=attendance-recovery&source=attendance&trainee=${traineeParam}`}
              className="flex items-center justify-between rounded-lg border border-[#a8a29e]/6 bg-black/10 px-3 py-3 text-sm transition hover:border-emerald-500/20 hover:bg-black/20"
            >
              <div>
                <p className="font-medium text-[#fafaf9]">Open catch-up learning path</p>
                <p className="mt-1 text-[#57534e]">Route this trainee into condensed recovery modules tied to attendance slippage.</p>
              </div>
              <ArrowRight className="h-4 w-4 text-emerald-400" />
            </Link>
            <Link
              href={`/report-studio?template=batch-intervention-pack&source=attendance&trainee=${traineeParam}&batch=${batchParam}`}
              className="flex items-center justify-between rounded-lg border border-[#a8a29e]/6 bg-black/10 px-3 py-3 text-sm transition hover:border-emerald-500/20 hover:bg-black/20"
            >
              <div>
                <p className="font-medium text-[#fafaf9]">Create intervention report</p>
                <p className="mt-1 text-[#57534e]">Use attendance behavior and AI insight as the starting point for a trainer handoff.</p>
              </div>
              <ArrowRight className="h-4 w-4 text-emerald-400" />
            </Link>
          </div>
