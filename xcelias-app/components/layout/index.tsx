...
const navSections: Array<{ title: string; items: NavItem[] }> = [
  {
    title: "Overview",
    items: [
      { href: "/", label: "Xcelias Home", shortLabel: "Home", icon: Crown, tone: "var(--signal-aqua)" },
      { href: "/command-center", label: "Command Center", shortLabel: "Center", icon: LayoutDashboard, tone: "var(--signal-aqua)" },
      { href: "/analytics", label: "Intelligence", shortLabel: "Intel", icon: BarChart3, tone: "var(--signal-violet)" },
      { href: "/market-intelligence", label: "Market Intel", shortLabel: "Market", icon: Search, tone: "var(--signal-amber)" },
    ],
  },
  {
    title: "Academy",
    items: [
      { href: "/learning-hub", label: "Learning Hub", shortLabel: "Learn", icon: Sparkles, tone: "var(--signal-mint)" },
      { href: "/report-studio", label: "Report Studio", shortLabel: "Reports", icon: ClipboardList, tone: "var(--signal-rose)" },
...
function getActiveTitle(pathname: string) {
  ...
  return match?.label ?? "Xcelias Home";
}
            ...
            <AnimatePresence>
              {sidebarOpen ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-white">{user?.name ?? "Xcelias Operator"}</p>
