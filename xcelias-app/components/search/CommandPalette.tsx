...
import { Search, RefreshCw, Users, GraduationCap, Building2, Sparkles } from "lucide-react";
import { commandShortcuts } from "@/lib/xcelias-data";

type SearchResult = {
  type: "trainee" | "batch" | "company" | "shortcut";
  id: string;
  title: string;
  subtitle?: string;
  url: string;
...
export function CommandPalette() {
  ...
  const shortcutResults = React.useMemo(
    () =>
  ...
  const filteredShortcuts = React.useMemo(() => {
    ...
    if (!value) return shortcutResults;

    return shortcutResults.filter(
      (item) =>
        item.title.toLowerCase().includes(value) ||
        (item.subtitle || "").toLowerCase().includes(value)
    );
  }, [query, shortcutResults]);
            ...
            <div className="max-h-80 overflow-y-auto p-2">
              {query.trim().length < 2 ? (
                <div className="px-4 py-8 text-center">
                  <p className="text-sm text-[#57534e]">Quick launch into Xcelias surfaces or type 2 characters to search live academy data.</p>
                  <div className="mt-5 space-y-2 text-left">
                    {shortcutResults.map((shortcut, index) => (
                      <button
                        key={shortcut.id}
                        type="button"
                        ...
                        className={`flex w-full items-center gap-4 rounded-xl px-4 py-3 transition ${
                          index === selectedIndex ? "bg-emerald-500/10 text-white" : "text-[#d6d3d1] hover:bg-[#231f1d]/50"
                        }`}
                      >
                        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[rgba(66,211,255,0.12)] text-[var(--signal-aqua)]">
                          <Sparkles className="h-5 w-5" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-medium">{shortcut.title}</p>
                          <p className="truncate text-sm text-[#57534e]">{shortcut.subtitle}</p>
                        </div>
                        <span className="rounded-full bg-[#231f1d]/50 px-2 py-0.5 text-xs text-[#57534e]">surface</span>
                      </button>
                    ))}
                  </div>
                </div>
              ) : !loading && results.length === 0 && filteredShortcuts.length === 0 ? (
                <div className="px-4 py-8 text-center">
                ...
                <div className="space-y-1">
                  ...
                  {results.map((r, i) => (
                    <motion.button
                      key={`${r.type}-${r.id}`}
                      type="button"
                      onMouseEnter={() => setSelectedIndex(i + filteredShortcuts.length)}
                      onClick={() => go(r)}
                      initial={{ opacity: 0, x: -10 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: (i + filteredShortcuts.length) * 0.02 }}
                      className={`flex w-full items-center gap-4 rounded-xl px-4 py-3 text-left transition ${
                        i + filteredShortcuts.length === selectedIndex ? "bg-emerald-500/10 text-white" : "text-[#d6d3d1] hover:bg-[#231f1d]/50"
                      }`}
                    >
                      <div
            ...
            {results.length > 0 && (
              <div className="border-t border-[#a8a29e]/6 px-4 py-2">
                <div className="flex items-center justify-between text-xs text-[#57534e]">
                  <span>{results.length + filteredShortcuts.length} results</span>
